...
internal class StorageRootFetcher(private val context: Context, private val isRestore: Boolean) {
    ...
    private fun checkOrAddNextCloudRoot(roots: ArrayList<StorageRoot>) {
        ...
        val marketIntent = Intent(ACTION_VIEW, Uri.parse("market://details?id=$NEXTCLOUD_PACKAGE")).apply {
            addFlags(FLAG_ACTIVITY_NEW_TASK)
        }
        ...
        val summaryRes = if (isInstalled) {
            ...
            if (!canInstall) R.string.storage_fake_nextcloud_summary_unavailable_market
        ...
        val root = StorageRoot(
            ...
            enabled = (!isInstalled && canInstall) || isRestore,
